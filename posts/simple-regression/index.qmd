...
date: "2024-02-18"
categories: [505, regression, total squared error, least squared error, analysis of regression]
execute:
    freeze: true
---
...
- $b_1$ is the slope or regression coefficient (how much $Y$ will change when $X$ increases by 1)  
  - larger values of $b_1$ = steeper regression line
...
- $\hat{Y}$ is the _predicted_ value of $Y$
...
- right image, all slopes positive, but different intercepts
...
- $\hat{Y}$ should be close to actual $Y$
  - difference between the two is considered `prediction error` (distance between the line and the data point)

![graph showing the difference between predicted and actual values of $Y$](predicted.png)

- green dots = $\hat{Y}$
- blue dots = $Y$
- distance between the green and blue dots is the `prediction error` 
  - need to minimize
  - `0` prediction error is very unlikely
...
- for each possible line, we can calculate the `total squared error`, then choose the line with the lowest total squared error, which is the `regression line`
...
- Estimated slope -> $\hat{b_1}=r\frac{S_Y}{S_X}$
- Estimated intercept -> $\hat{b_0}=M_Y-\hat{b_1}M_x$
...
- standardized slope ($b_1$) = 1
- standardized intercept ($b_0$) = 0
...
$\hat{Z_{X_i}}$ is the standard deviation of $X$  
...
- difference between observed data $Y$, and the mean of the data $\bar{Y}$

`Sum Square Residual` $SS_R$ (right plot) is $\sum(Y-\hat{Y})^2$
- difference between observed data $Y$ and the regression line $\hat{Y}$, or predicted data

- `prediction error` 
  - squared distance between the data and the regression line
...
- difference between predicted quantity $\hat{Y}$ and the mean of the data $\bar{Y}$
...
- $SS_M$ or sum square regression: Model variability - difference in variability between the model and the mean.$Y$ are determined by the value of $X$  
  - if $b_1$ is positive, then larger $X$ will mean larger $Y$
...
- record company boss interested in predicting album sales from advertising based on 200 different album releases  
...
![F=99.587 means that the variance explained by the regression model is 99.587 times the variance explained by the residual model. F-test is significant.](anova.png)
